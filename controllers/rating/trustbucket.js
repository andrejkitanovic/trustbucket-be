...
exports.getTrustbucketReviews = async (req, res, next) => {
    try {
        ...
        const company = await Company.findOne({
            ...
            'image name websiteURL email phone address socialLinks ratings reviewsPageLanguage'
        )
...
exports.postTrustbucketReviews = async (req, res, next) => {
    try {
        const { slug, rating, title, description, image, name, email } =
            req.body
        ...
        if (!req.body.campaignId) {
            ...
            campaign.trustbucketRating =
                (campaign.trustbucketRating * campaign.verifiedReviews +
                    rating) /
                (campaign.verifiedReviews + 1)
            campaign.verifiedReviews += 1
            await campaign.save()
            ...
            const avarageRating = allRatings.reduce(
                (total, el) => total + el.rating,
                0
            )
            const totalRatingCount = await Rating.countDocuments({
                company: company._id,
                type: 'trustbucket',
            })
