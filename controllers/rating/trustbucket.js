...
exports.postTrustbucketReviews = async (req, res, next) => {
  try {
    const {
      slug, rating, title, description, image, name, email,
    } = req.body;
    ...
    if (!req.body.campaignId) {
      ...
      campaign.trustbucketRating =				(campaign.trustbucketRating * campaign.verifiedReviews + rating) / (campaign.verifiedReviews + 1);
