...
exports.searchTrustpilotProfile = async (req, res, next) => {
  ...
  try {
    ...
    const jsonParse = await JSON.parse(
      $('script[type="application/ld+json"][data-business-unit-json-ld]').html()
    )
    const json = jsonParse['@graph'].find(
      (object) => object['@type'] === 'LocalBusiness'
    )
...
exports.cronTrustpilotProfile = async (
  url,
  selectedCompany,
  previousRatings
) => {
