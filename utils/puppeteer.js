    ...
    try {
        ...
        await $('.modal-content div[itemprop=review]').map((index, el) => {
            ...
            const object = {
                ...
                rating: Number(
                    $el('meta[itemprop=ratingValue]').attr('content')
                ),
                description: $el('div.review-text').text(),
                date: dayjs(
                    $el('time[datetime]').attr('datetime'),
                    'YYYY-MM-DD'
                ),
            }
...
const getHittaReviews = async ({ page, url, selectedCompany }) => {
    try {
        ...
        await $('div.section-block--review').map((index, el) => {
            ...
            const reviewEl = $el(
                'div.widget-header_rating-svg_container > div'
            ).attr('style')
            if (reviewEl) {
                object.rating =
                    parseInt(
    ...
    try {
        ...
        await $('div[data-qa=reviews-list] li').forEach((index, el) => {
            ...
            const object = {
                ...
                rating: Number(
                    $el('div[data-qa=review-rating]').count(
                        'div[type=selected]'
                    )
                ),
                description: $el('p[class*=StyledParagraph]').text(),
                date: dayjs(
                    $el('p[data-qa=review-appt-date]').text(),
                    'MMM D, YYYY'
                ),
            }
...
exports.getCluster = async () => {
    ...
    await cluster.task(async ({ page, data }) => {
        ...
        if (items.length) {
            ...
            items = items.filter(
                (item) => item.name && item.rating && item.date
            )
